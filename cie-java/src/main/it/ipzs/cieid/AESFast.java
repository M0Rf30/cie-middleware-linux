 ...
 *
 * <p>This implementation is based on optimizations from Dr. Brian Gladman's paper and C code at <a
 ...
 *
 * <p>There are three levels of tradeoff of speed vs memory Because java has no preprocessor, they
 * are written as three separate classes from which to choose
 *
 * <p>The fastest uses 8Kbytes of static tables to precompute round calculations, 4 256 word tables
 * for encryption and 4 for decryption.
 *
 * <p>The middle performance version uses only one 256 word table for each, for a total of 2Kbytes,
 * adding 12 rotate operations per round to compute the values contained in the other tables from
 * the contents of the first
 *
 * <p>The slowest version uses no static tables at all and computes the values in each round
 *
 * <p>This file contains the fast version with 8Kbytes of static tables for round precomputation
 */
public class AESFast {

  public static final int KEY_SIZE_256 = 256;
  public static final int KEY_SIZE_224 = 224;
  public static final int KEY_SIZE_160 = 160;
  public static final int KEY_SIZE_192 = 192;
  public static final int KEY_SIZE_128 = 128;

  public static final int BLOCK_SIZE_128 = 128;
  ...
  */
   ...
   * size AES specified a fixed block size of 128 bits and key sizes 128/192/256 bits This code is
   * written assuming those are the only possible values
   */
  private int[][] generateWorkingKey(byte[] key, boolean forEncryption) {
    ...
    ROUNDS = KC + 6; // This is not always true for the generalized Rijndael that allows
    // larger block sizes
  ...
  private final void encryptBlock(int[][] KW) {
    ...
    r0 =
        T0[C0 & 255] ^ T1[(C1 >> 8) & 255] ^ T2[(C2 >> 16) & 255] ^ T3[(C3 >> 24) & 255] ^ KW[r][0];
    r1 =
        T0[C1 & 255] ^ T1[(C2 >> 8) & 255] ^ T2[(C3 >> 16) & 255] ^ T3[(C0 >> 24) & 255] ^ KW[r][1];
    r2 =
        T0[C2 & 255] ^ T1[(C3 >> 8) & 255] ^ T2[(C0 >> 16) & 255] ^ T3[(C1 >> 24) & 255] ^ KW[r][2];
    r3 =
        T0[C3 & 255]
            ^ T1[(C0 >> 8) & 255]
            ^ T2[(C1 >> 16) & 255]
    ...
    // the final round's table is a simple function of S so we don't use a whole other four
    // tables for it
  ...
  private final void decryptBlock(int[][] KW) {
    ...
    // the final round's table is a simple function of Si so we don't use a whole other four
    // tables for it
  ...
  //     * Encrypt a palintext.
  //     *
  //     * @param  plaintext The plaintex.
  //     * @param  nOffset   Index of in from which to start considering data.
  //     * @param  sessionKey The session key to use for encryption.
  //     * @param  blockSize  The block size in bytes of this Rijndael.
  //     * @return The ciphertext generated using the session key.
  //     */
  //    public byte[] encryptECB(byte[] plaintext, int nOffset)
  //    {
  ////    	LogViewer.logMsg("blocksize " + getBlockSize());
  ////    	LogViewer.logMsg("to pad " + Encoder.bytesToHexString(plaintext));
  //        // pad
  //        plaintext = pad(plaintext, nOffset);
  //
  ////        LogViewer.logMsg("padded " + Encoder.bytesToHexString(plaintext));
  //
  //        byte[] encblock = new byte[getBlockSize()];
  //
  //        ByteArrayOutputStream outsCipherText = new ByteArrayOutputStream();
  //        while(nOffset < plaintext.length)
  //        {
  ////        	LogViewer.logMsg("plaintext " + Encoder.bytesToHexString(plaintext));
  //
  //            processBlock(plaintext, nOffset, encblock, 0);
  //            outsCipherText.write(encblock, 0, getBlockSize());
  //            nOffset += getBlockSize();
  //        }
  //
  ////        LogViewer.logMsg("ciphertext " +
  // Encoder.bytesToHexString(outsCipherText.toByteArray()));
  //
   ...
   * @param sessionKey The session key to use for encryption.
   * @param blockSize The block size in bytes of this Rijndael.
   * @return The plaintext generated from ciphertext using the session key.
   */
  public byte[] decryptECB(byte[] ciphertext, int nOffset) {
    //    	System.gc();

    //    	LogViewer.logMsg("ciphertext " + Encoder.bytesToHexString(ciphertext));
    ...
    try {
      while (nOffset < ciphertext.length) {
        //            	LogViewer.logMsg("cipher block " +
        // Encoder.bytesToHexString(ciphertext, nOffset, getBlockSize()));
        processBlock(ciphertext, nOffset, block, 0);
        //                LogViewer.logMsg("clear block " +
        // Encoder.bytesToHexString(block));
        outs.write(block, 0, getBlockSize());
        nOffset += getBlockSize();
      }

      //            LogViewer.logMsg("to unpad " +
      // Encoder.bytesToHexString(outs.toByteArray()));

      return unpad(outs.toByteArray());
    } catch (RuntimeException e) {
      //        	LogViewer.logException(e);

      throw new AESException();
    }
  }

  //    public byte[] encryptCBC(byte[] plaintext, int nOffset, byte[] iv)
  //    {
  //        // pad
  //        byte[] encblock = new byte[getBlockSize()];
  //        byte[] xoredblock = new byte[getBlockSize()];
  //        byte[] block = pad(plaintext, nOffset);
  //
  //        // copy IV
  //        System.arraycopy(iv, 0, encblock, 0, encblock.length);
  //
  //        ByteArrayOutputStream outsCipherText = new ByteArrayOutputStream();
  //        while(nOffset < block.length)
  //        {
  ////            System.out.println(Encoder.bytesToHexString(block));
  //
  //            xor(block, nOffset, encblock, 0, xoredblock, 0, getBlockSize());
  //
  ////            System.out.println(Encoder.bytesToHexString(block));
  //
  //            processBlock(xoredblock, 0, encblock, 0);
  //
  ...
  public byte[] decryptCBC(byte[] ciphertext, int nOffset, byte[] iv) {
    ...
    byte[] tmpiv = new byte[getBlockSize()];
    System.arraycopy(iv, 0, tmpiv, 0, getBlockSize());

    while (nOffset < ciphertext.length) {
      processBlock(ciphertext, nOffset, xoredblock, 0);

      //            System.out.println(Encoder.bytesToHexString(block));

      xor(xoredblock, 0, tmpiv, 0, block, 0, getBlockSize());

      //            System.out.println(Encoder.bytesToHexString(block));

      outs.write(block, 0, getBlockSize());

      System.arraycopy(ciphertext, nOffset, tmpiv, 0, getBlockSize());
  ...
  //    private byte[] pad(byte[] plaintext, int nOffset)
  //    {
  //        ByteArrayOutputStream outsPlainText = new ByteArrayOutputStream();
  //        Random rnd = new Random();
  //        rnd.setSeed(System.currentTimeMillis());
  //        byte btRnd[] = new byte[1];
  //        // add padding
  //        int nPadLen = (getBlockSize() - ((plaintext.length + 1 - nOffset) % getBlockSize()));
  //        outsPlainText.write(nPadLen);
  //        for(int i = 0; i < nPadLen - 1; i++)
  //        {
  //        	rnd.nextBytes(btRnd);
  //            outsPlainText.write((char)btRnd[0]);
  //        }
  //
  //        outsPlainText.write((char)0x00);
  //
  //        // write plaintext
  //        outsPlainText.write(plaintext, nOffset, plaintext.length - nOffset);
  //
  ...
  private byte[] unpad(byte[] plaintext) {
    ...
    //        LogViewer.logMsg("plainText[nPadLen] " + plaintext[nPadLen]);
    //        LogViewer.logMsg(Encoder.bytesToHexString(plaintext));
    // #endif

    if (plaintext[nPadLen] != 0x00) throw new IllegalArgumentException();
  ...
  //     * @return The ciphertext generated using the session key.
  //     */
  //    public static byte[] encryptECB(byte[] plaintext, int nOffset, byte[] key, int keylen, int
  // nBlockLen)
  //    {
  //    	AESFast aes = new AESFast();
  //    	aes.init(true, makeSafeKey(key, keylen));
  //        return aes.encryptECB(plaintext, nOffset);
  //    }
  //
  /**
  ...
  public static byte[] decryptECB(
      byte[] ciphertext, int nOffset, byte[] key, int keylen, int nBlockLen) {
    AESFast aes = new AESFast();
    aes.init(false, makeSafeKey(key, keylen));
  ...
  //    	aes.init(true, makeSafeKey(key, keylen));
  //
  //        return aes.encryptCBC(plaintext, nOffset, makeSafeKey(iv, nBlockLen));
  //    }
  ...
  public static byte[] decryptCBC(
      byte[] ciphertext, int nOffset, byte[] key, int keylen, byte[] iv, int nBlockLen) {
    AESFast aes = new AESFast();
    // aes.init(false, makeSafeKey(key, keylen));
    aes.init(false, key);

    return aes.decryptCBC(ciphertext, nOffset, iv);
  }

  public static byte[] makeSafeKey(byte[] k, int keysize) {
    ByteArrayOutputStream outs = new ByteArrayOutputStream(keysize);

    int nDiff = (keysize / 8) - k.length;
    try {
      if (nDiff > 0) {
        outs.write(k);
        for (int i = 0; i < nDiff; i++) outs.write(k[i % k.length]);
      } else if (nDiff < 0) {
        int len = k.length + nDiff;
        for (int i = 0; i < k.length + nDiff; i++) outs.write(k[i]);
      } else {
        outs.write(k);
      }
    } catch (IOException e) {
  ...
  //
  //        byte[] enc = encryptCBC(msg.getBytes(), 0, key.getBytes(), KEY_SIZE_256,
  // iv.getBytes(), BLOCK_SIZE_128);
  //        System.out.println(Encoder.bytesToHexString(enc));
  //
  //        byte[] dec = decryptCBC(enc, 0, key.getBytes(), KEY_SIZE_256, iv.getBytes(),
  // BLOCK_SIZE_128);
  //        System.out.println(new String(dec));
  //
  //        enc = encryptECB(msg.getBytes(), 0, key.getBytes(), KEY_SIZE_256, BLOCK_SIZE_128);
  //        System.out.println(Encoder.bytesToHexString(enc));
  //
  //        dec = decryptECB(enc, 0, key.getBytes(), KEY_SIZE_256, BLOCK_SIZE_128);
  //        System.out.println(new String(dec));
  //
  //    }
}
