...
extern "C" {
CK_RV CK_ENTRY AbilitaCIE(const char* szPAN, const char* szPIN, int* attempts,
                          PROGRESS_CALLBACK progressCallBack,
...
CK_RV CK_ENTRY DisabilitaCIE(const char* szPAN) {
  if (IAS::IsEnrolled(szPAN)) {
    ...
    LOG_ERROR(
        "DisabilitaCIE - Unable to remove CIE number %s, CIE is not enrolled",
        szPAN);
...
CK_RV CK_ENTRY AbilitaCIE(const char* szPAN, const char* szPIN, int* attempts,
  ...
  try {
    ...
    for (; curreader[0] != 0; curreader += strnlen(curreader, len) + 1) {
      ...
      LOG_INFO("AbbinaCIE - Verifying SOD, digest algorithm: %s",
...
int TokenTransmitCallback(safeConnection* conn, BYTE* apdu, DWORD apduSize,
  ...
  if (apduSize == 2) {
    ...
    if (code == 0xfffd) {
      ...
      *respSize = sizeof(conn->hCard) + 2;
      ...
      auto ris =
          SCardReconnect(conn->hCard, SCARD_SHARE_SHARED, SCARD_PROTOCOL_Tx,
      ...
      DWORD protocol = 0;
      auto ris = SCardReconnect(conn->hCard, SCARD_SHARE_SHARED,
  ...
  auto ris = SCardTransmit(conn->hCard, SCARD_PCI_T1, apdu, apduSize, NULL,
                           resp, respSize);
  ...
  if (ris == SCARD_W_RESET_CARD || ris == SCARD_W_UNPOWERED_CARD) {
    ...
    DWORD protocol = 0;
    ris = SCardReconnect(conn->hCard, SCARD_SHARE_SHARED, SCARD_PROTOCOL_Tx,
                         SCARD_LEAVE_CARD, &protocol);
    if (ris != SCARD_S_SUCCESS)
      LOG_ERROR("TokenTransmitCallback - ScardReconnect error: %d");
    else {
      ris = SCardTransmit(conn->hCard, SCARD_PCI_T1, apdu, apduSize, NULL, resp,
                          respSize);
      LOG_DEBUG("TokenTransmitCallback - Smart card response:");
      LOG_BUFFER(resp, *respSize);
    }
...
std::vector<word32> fromObjectIdentifier(std::string sObjId) {
  ...
  UINT nFirst = 40 * strtol(szTok, NULL, 10) +
                strtol(strtok_r(NULL, ".", &next), NULL, 10);
  if (nFirst > 0xff) {
    delete[] szOID;
    throw -1;
  }
...
void notifyPINLocked() { sendMessage("pinlocked", NULL); }
