...
uint16_t CIESign::sign(const char* inFilePath, const char* type,
  ...
  try {
    ...
    if (strcmp(type, "pdf") == 0) {
      ret = disigon_sign_set(ctx, DISIGON_OPT_PDF_SUBFILTER,
                             (void*)DISIGON_PDF_SUBFILTER_ETSI_CADES);
      if (ret != 0) {
        throw ret;
      }
      ...
      if (imagePathFile) {
        ret = disigon_sign_set(ctx, DISIGON_OPT_PDF_IMAGEPATH,
                               (void*)imagePathFile);
        if (ret != 0) {
          throw ret;
        }
      ...
      ret = disigon_sign_set(ctx, DISIGON_OPT_INPUTFILE_TYPE,
                             (void*)DISIGON_FILETYPE_PDF);
      if (ret != 0) {
        throw ret;
      }
    } else {
      if ((strstr(inFilePath, "p7m") != NULL) ||
          (strstr(inFilePath, "p7s") != NULL))
        ret = disigon_sign_set(ctx, DISIGON_OPT_INPUTFILE_TYPE,
                               (void*)DISIGON_FILETYPE_P7M);
      else
        ret = disigon_sign_set(ctx, DISIGON_OPT_INPUTFILE_TYPE,
                               (void*)DISIGON_FILETYPE_PLAINTEXT);
