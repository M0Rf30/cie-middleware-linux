...
class resetter<std::unique_ptr<T>> {
 ...
 public:
  ...
  void release() noexcept { m_p = nullptr; }
};
...
CK_SESSION_HANDLE CSession::AddSession(std::unique_ptr<CSession> pSession) {
  ...
  pSession->pSlot->pTemplate->FunctionList.templateInitSession(
      pSession->pSlot->pTemplateData);

  pSession->pSlot->dwSessionCount++;
  g_mSessions.insert(
      std::make_pair(pSession->hSessionHandle, std::move(pSession)));
...
void CSession::DeleteSession(CK_SESSION_HANDLE hSessionHandle) {
  ...
  pSession->pSlot->pTemplate->FunctionList.templateFinalSession(
      pSession->pSlot->pTemplateData);
...
void CSession::Login(CK_USER_TYPE userType, CK_CHAR_PTR pPin,
  ...
  if (userType == CKU_SO) {
    if (ExistsRO()) throw p11_error(CKR_SESSION_READ_ONLY_EXISTS);
  }
  if (pSlot->User != CKU_NOBODY) throw p11_error(CKR_USER_ALREADY_LOGGED_IN);

  ByteArray baPin(pPin, ulPinLen);
  pSlot->pTemplate->FunctionList.templateLogin(pSlot->pTemplateData, userType,
                                               baPin);
...
void CSession::Logout() {
  ...
  for (auto obj = pSlot->P11Objects.begin(); obj != pSlot->P11Objects.end();
       obj++) {
  ...
  if (ulCount == 0) {
    for (auto obj = pSlot->P11Objects.begin(); obj != pSlot->P11Objects.end();
      ...
      try {
        ...
        if (err.getP11ErrorCode() != CKR_USER_NOT_LOGGED_IN) throw;
      }
    }
  } else {
    for (auto obj = pSlot->P11Objects.begin(); obj != pSlot->P11Objects.end();
        ...
        try {
          ...
          if (attr == nullptr)
            ...
            if (attr->size() != pTemplate[j].ulValueLen) {
              bSkip = true;
            } else if ((*attr) != ByteArray((BYTE *)pTemplate[j].pValue,
...
CK_OBJECT_HANDLE CSession::CreateObject(CK_ATTRIBUTE_PTR pTemplate,
  ...
  std::shared_ptr<CP11Object> pObject =
      pSlot->pTemplate->FunctionList.templateCreateObject(pSlot->pTemplateData,
                                                          pTemplate, ulCount);
...
void CSession::DestroyObject(CK_OBJECT_HANDLE hObject) {
  ...
  pSlot->pTemplate->FunctionList.templateDestroyObject(pSlot->pTemplateData,
                                                       *pObject);
...
void CSession::DigestInit(CK_MECHANISM_PTR pMechanism) {
  ...
  switch (pMechanism->mechanism) {
    case CKM_SHA_1: {
      auto mech =
          std::unique_ptr<CDigestSHA>(new CDigestSHA(shared_from_this()));
      mech->DigestInit();
    ...
    case CKM_SHA256: {
      auto mech =
          std::unique_ptr<CDigestSHA256>(new CDigestSHA256(shared_from_this()));
      mech->DigestInit();
    ...
    case CKM_MD5: {
      auto mech =
          std::unique_ptr<CDigestMD5>(new CDigestMD5(shared_from_this()));
      mech->DigestInit();
...
void CSession::VerifyInit(CK_MECHANISM_PTR pMechanism, CK_OBJECT_HANDLE hKey) {
  ...
  switch (pMechanism->mechanism) {
    case CKM_SHA1_RSA_PKCS: {
      auto mech =
          std::unique_ptr<CRSAwithSHA1>(new CRSAwithSHA1(shared_from_this()));
      mech->VerifyInit(hKey);
      pVerifyMechanism = std::move(mech);
      break;
    }
    case CKM_SHA256_RSA_PKCS: {
      auto mech = std::unique_ptr<CRSAwithSHA256>(
          new CRSAwithSHA256(shared_from_this()));
      mech->VerifyInit(hKey);
      pVerifyMechanism = std::move(mech);
      break;
    }
    case CKM_MD5_RSA_PKCS: {
      auto mech =
          std::unique_ptr<CRSAwithMD5>(new CRSAwithMD5(shared_from_this()));
      mech->VerifyInit(hKey);
      pVerifyMechanism = std::move(mech);
      break;
    }
    case CKM_RSA_PKCS: {
      auto mech =
          std::unique_ptr<CRSA_PKCS1>(new CRSA_PKCS1(shared_from_this()));
      mech->VerifyInit(hKey);
      pVerifyMechanism = std::move(mech);
      break;
    }
...
void CSession::VerifyRecoverInit(CK_MECHANISM_PTR pMechanism,
  ...
  std::shared_ptr<CP11Object> pObject = pSlot->GetObjectFromID(hKey);
  if (pObject == NULL) throw p11_error(CKR_KEY_HANDLE_INVALID);
  if (pObject->ObjClass != CKO_PUBLIC_KEY)
    throw p11_error(CKR_KEY_HANDLE_INVALID);
...
void CSession::SignFinal(ByteArray &Signature) {
  ...
  std::shared_ptr<CP11Object> pObject =
      pSlot->GetObjectFromID(pSignMechanism->hSignKey);
  if (pObject == NULL) throw p11_error(CKR_KEY_HANDLE_INVALID);
  if (pObject->ObjClass != CKO_PRIVATE_KEY)
    throw p11_error(CKR_KEY_HANDLE_INVALID);
  ...
  pSlot->pTemplate->FunctionList.templateSign(
      pSlot->pTemplateData, pSignKey.get(), baSignBuffer, baSignature,
      pSignMechanism->mtType, bSilent);
...
void CSession::SignRecover(ByteArray &Data, ByteArray &Signature) {
  ...
  std::shared_ptr<CP11Object> pObject =
      pSlot->GetObjectFromID(pSignRecoverMechanism->hSignRecoverKey);
  if (pObject == nullptr) throw p11_error(CKR_KEY_HANDLE_INVALID);
  if (pObject->ObjClass != CKO_PRIVATE_KEY)
    throw p11_error(CKR_KEY_HANDLE_INVALID);
  auto pSignRecoverKey = std::static_pointer_cast<CP11PrivateKey>(pObject);
  ...
  ByteDynArray baSignature;
  pSlot->pTemplate->FunctionList.templateSignRecover(
      pSlot->pTemplateData, pSignRecoverKey.get(), baSignRecoverBuffer,
      baSignature, pSignRecoverMechanism->mtType, bSilent);
...
void CSession::SetAttributeValue(CK_OBJECT_HANDLE hObject,
  ...
  pSlot->pTemplate->FunctionList.templateSetAttribute(
      pSlot->pTemplateData, pObject.get(), pTemplate, ulCount);
}
...
void CSession::SetOperationState(ByteArray &OperationState) {
  ...
  if (User.isNull()) throw p11_error(CKR_SAVED_STATE_INVALID);

  if (Flags != VarToByteArray(flags)) throw p11_error(CKR_SAVED_STATE_INVALID);
  ...
  if (!SignOperationState.isNull()) {
    ...
    if (Algo.isNull()) throw p11_error(CKR_SAVED_STATE_INVALID);
    CK_MECHANISM mech = {ByteArrayToVar(Algo, CK_MECHANISM_TYPE), NULL, 0};

    ByteArray Key = SignTlv.getValue(OS_Key);
    if (Key.isNull()) throw p11_error(CKR_SAVED_STATE_INVALID);

    std::shared_ptr<CP11Object> pKey = pSlot->FindP11Object(
        CKO_PRIVATE_KEY, CKA_ID, Key.data(), (int)Key.size());
    ...
    if (Data.isNull()) throw p11_error(CKR_SAVED_STATE_INVALID);
  ...
  if (!VerifyOperationState.isNull()) {
    ...
    std::shared_ptr<CP11Object> pKey = pSlot->FindP11Object(
        CKO_PUBLIC_KEY, CKA_ID, Key.data(), (int)Key.size());
    ER_ASSERT(pKey != nullptr, ERR_CANT_GET_OBJECT)
    CK_OBJECT_HANDLE hKey = pSlot->GetIDFromObject(pKey);
...
void CSession::GetOperationState(ByteArray &OperationState) {
  ...
  if (pSignMechanism) {
    CTLVCreate SignTlv;
    ByteArray ba3((BYTE *)&pSignMechanism->mtType,
                  sizeof(pSignMechanism->mtType));
    SignTlv.setValue(OS_Algo, ba3);
    ByteDynArray SignData = pSignMechanism->SignGetOperationState();
    if (!SignData.isEmpty()) SignTlv.setValue(OS_Data, SignData);
  ...
  if (pVerifyMechanism) {
    ...
    ByteArray ba((BYTE *)&pVerifyMechanism->mtType,
                 sizeof(pVerifyMechanism->mtType));
    VerifyTlv.setValue(OS_Algo, ba);
    ByteDynArray verifyData = pVerifyMechanism->VerifyGetOperationState();
    if (!verifyData.isEmpty()) VerifyTlv.setValue(OS_Data, verifyData);

    std::shared_ptr<CP11Object> pKey =
        pSlot->GetObjectFromID(pVerifyMechanism->hVerifyKey);
    ER_ASSERT(pKey != nullptr, ERR_CANT_GET_OBJECT);
  ...
  if (pDigestMechanism) {
    ...
    ByteArray ba((BYTE *)&pDigestMechanism->mtType,
                 sizeof(pDigestMechanism->mtType));
    DigestTlv.setValue(OS_Algo, ba);
    ByteDynArray DigestData = pDigestMechanism->DigestGetOperationState();
    if (!DigestData.isEmpty()) DigestTlv.setValue(OS_Data, DigestData);
