...
static DWORD slotMonitor(SlotMap *pSlotMap) {
  while (true) {
    ...
    DWORD ris;
    {
      ...
      for (SlotMap::const_iterator it = pSlotMap->begin();
           it != pSlotMap->end(); it++, i++) {
        if (!bP11Initialized) {
...
CK_SLOT_ID CSlot::AddSlot(std::shared_ptr<CSlot> pSlot) {
  init_func pSlot->hSlot = (CK_SLOT_ID)pSlot->GetNewSlotID();
  auto id = pSlot->hSlot;
...
std::shared_ptr<CSlot> CSlot::GetSlotFromReaderName(const char *name) {
  init_func for (SlotMap::iterator it = g_mSlots.begin(); it != g_mSlots.end();
                 it++) {
...
std::shared_ptr<CSlot> CSlot::GetSlotFromID(CK_SLOT_ID hSlotId) {
  init_func SlotMap::const_iterator pPair;
...
void CSlot::InitSlotList() {
  ...
  if (ris != S_OK) {
    if (ris == SCARD_E_NO_READERS_AVAILABLE) return;
    throw windows_error(ris);
  }
  std::string readers;
  readers.resize(readersLen + 1);
  if ((ris = SCardListReaders(Context, NULL, &readers[0], &readersLen)) !=
      SCARD_S_SUCCESS)
    throw windows_error(ris);
...
void CSlot::GetInfo(CK_SLOT_INFO_PTR pInfo) {
  ...
  // non so esattamente perchè, ma nella R1 il manufacturerID sono i primi 32
  // dello slotDescription
...
void CSlot::DelP11Object(const std::shared_ptr<CP11Object> &object) {
  ...
  for (P11ObjectVector::iterator it = P11Objects.begin();
       it != P11Objects.end(); it++) {
    if (*it == object) {
      bFound = true;
      P11Objects.erase(it);
      break;
...
size_t CSlot::RWSessionCount() {
  ...
  for (SessionMap::iterator it = CSession::g_mSessions.begin();
       it != CSession::g_mSessions.end(); it++) {
    if (it->second->pSlot.get() == this &&
...
void CSlot::DelObjectHandle(const std::shared_ptr<CP11Object> &pObject) {
  init_func ObjHandleMap::iterator pPair;
  pPair = ObjP11Map.find(pObject);
  if (pPair != ObjP11Map.end()) {
...
std::shared_ptr<CP11Object> CSlot::GetObjectFromID(
  ...
  init_func HandleObjMap::const_iterator pPair;
...
void CSlot::Connect() {
  ...
  bool retry = false;
  while (true) {
    DWORD ris = SCardConnect(Context, szName.c_str(), SCARD_SHARE_SHARED,
...
ByteDynArray CSlot::GetATR() {
  init_func
      ...
      DWORD atrLen = 40;
  char ATR[40];
  long ret = SCardGetAttrib(this->hCard, SCARD_ATTR_ATR_STRING, (uint8_t *)ATR,
                            &atrLen);
