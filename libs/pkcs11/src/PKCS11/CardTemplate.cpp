...
void CCardTemplate::AddTemplate(std::shared_ptr<CCardTemplate> pTemplate) {
  init_func g_mCardTemplates.emplace_back(std::move(pTemplate));
}

void CCardTemplate::DeleteTemplateList() { init_func g_mCardTemplates.clear(); }
...
std::shared_ptr<CCardTemplate> CCardTemplate::GetTemplate(CSlot &pSlot) {
  init_func for (DWORD i = 0; i < g_mCardTemplates.size(); i++) {
    try {
