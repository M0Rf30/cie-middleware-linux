...
void Logger::debug(string& text) throw() { debug(text.data()); }
...
void Logger::info(string& text) throw() { info(text.data()); }
...
int Logger::error(string& text) throw() { return error(text.data()); }
...
void Logger::print_bytes(ostream& out, uint8_t* data, size_t dataLen,
                         bool format) {
  size_t index = 0;
