...
ByteArray::ByteArray(const ByteArray &ba, size_t start) {
  if (start > ba.size()) throw logged_error("Array derivato troppo grande");
...
int ByteArray::atoi() const {
  ...
  for (size_t i = 0; i < _size; i++) {
    if (val > (INT_MAX / 10)) throw std::overflow_error("owerflow");
...
void ByteArray::copy(const ByteArray &src, size_t start) {
  if (src._size + start > _size)
    throw logged_error(stdPrintf(
        "Dimensione array da copiare %i troppo grande; dimensione massima %i",
...
void ByteArray::rightcopy(const ByteArray &src, size_t end) {
  if (src._size + end > _size)
    throw logged_error(stdPrintf(
        "Dimensione array da copiare %i troppo grande; dimensione massima %i",
        src._size + end, _size));
  CryptoPP::memcpy_s(_data + _size - end - src._size, (end + src._size),
                     src._data, src._size);
}
...
ByteArray ByteArray::right(size_t size) const {
  if (size > _size) throw logged_error("Array derivato troppo grande");
...
void ByteDynArray::resize(size_t size, bool bKeepData) {
  if (!bKeepData) {
    ...
    if (dwMinSize > 0) CryptoPP::memcpy_s(pbtNewData, size, _data, dwMinSize);
    clear();
    _data = pbtNewData;
    _size = size;
  }
