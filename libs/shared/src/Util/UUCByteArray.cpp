...
UUCByteArray::UUCByteArray(const UUCByteArray& blob) : m_szHex(NULL) {
...
UUCByteArray::UUCByteArray(const char* szHexString) : m_szHex(NULL) {
...
UUCByteArray::UUCByteArray(const unsigned long nLen) : m_szHex(NULL) {
  // m_unLen = nLen;
  m_unLen = 0;
  m_nCapacity = nLen;
  // m_pbtContent = (BYTE*)GlobalAlloc(GPTR, m_nCapacity);
...
UUCByteArray::UUCByteArray() : m_szHex(NULL) {
  m_unLen = 0;
  m_nCapacity = DEFAULT_CAPACITY;
  // m_pbtContent = (BYTE*)GlobalAlloc(GPTR, m_nCapacity);
  m_pbtContent = (BYTE*)malloc(m_nCapacity);
...
long UUCByteArray::load(const char* szHexString) {
  ...
  // m_pbtContent = (BYTE*)GlobalAlloc(GPTR, m_nCapacity);
  m_pbtContent = (BYTE*)malloc(m_nCapacity);
  if (m_pbtContent == NULL) return ERROR_UNABLE_TO_ALLOCATE;
...
const BYTE* UUCByteArray::getContent() const { return m_pbtContent; }

unsigned long UUCByteArray::getLength() const { return m_unLen; }
...
void UUCByteArray::set(const unsigned int index,
...
long UUCByteArray::append(const BYTE btVal) {
  if (m_unLen == m_nCapacity) {
    ...
    if (buffer == NULL) return ERROR_UNABLE_TO_ALLOCATE;
...
void UUCByteArray::toHexString(char* szHexString) const
{
        ...
        {
                throw new UUCException("UUCByteArray:toHexString:Access
Violation");
        }
}
*/
...
long UUCByteArray::reverse() {
  ...
  if (pbtContent == NULL) return ERROR_UNABLE_TO_ALLOCATE;

  for (u_int64_t i = 0; i < m_unLen; i++) {
...
const char* UUCByteArray::toHexString() { return toHexString(0); }
