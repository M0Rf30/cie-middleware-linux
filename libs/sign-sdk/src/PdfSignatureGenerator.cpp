...
int PdfSignatureGenerator::Load(const char* pdf, int len) {
  ...
  try {
    ...
    // old code (commented for now cause we want one sign)
    // if (nSigns > 0) {
...
void PdfSignatureGenerator::InitSignature(
  ...
  // old-code maybe can be replaced
  if (szName && szName[0]) {
  ...
  if (szGraphometricData && szGraphometricData[0])
    SetGraphometricData(m_pSignatureField,
...
void PdfSignatureGenerator::SetAppearance(PoDoFo::PdfSignatureField* pSignatureField,
                     const char* szImagePath, const PdfString& szDescription, const bool showData = false) {
  PoDoFo::PdfObject* m_pSignatureObj = pSignatureField->GetFieldObject();

  if (!m_pSignatureObj) {
    PODOFO_RAISE_ERROR(ePdfError_InvalidHandle);
  }

  pSignatureField->SetBackgroundColorTransparent();
  PdfImage* pdfi = new PdfImage(m_pPdfDocument);
  pdfi->LoadFromPng(szImagePath);
  pdfi->SetImageChromaKeyMask(0,0,0);
  pSignatureField->SetAppearanceStream(pdfi,ePdfAnnotationAppearance_Normal, PdfName(""));

  if (m_pSignatureObj->GetDictionary().HasKey(PdfName("Description"))) {
    m_pSignatureObj->GetDictionary().RemoveKey(PdfName("Description"));
  }

  m_pSignatureObj->GetDictionary().AddKey(PdfName("Description"), szDescription);
}

void PdfSignatureGenerator::SetSignatureName(PoDoFo::PdfSignatureField* pSignatureField,
  ...
  m_pSignatureObj->GetDictionary().AddKey(PdfName("Name"), szName);
}

void PdfSignatureGenerator::SetGraphometricData(
    PdfSignatureField* pSignatureField, const PdfString& vendor,
    const PdfString& szGraphometricData, const PdfString& szVersion) {
  PoDoFo::PdfObject* m_pSignatureObj = pSignatureField->GetFieldObject();
  ...
  if (m_pSignatureObj->GetDictionary().HasKey(PdfName("GraphometricData"))) {
    m_pSignatureObj->GetDictionary().RemoveKey(PdfName("GraphometricData"));
  }

  m_pSignatureObj->GetDictionary().AddKey(PdfName("GraphometricData"),
                                          szGraphometricData);

  if (m_pSignatureObj->GetDictionary().HasKey(PdfName("Version"))) {
    m_pSignatureObj->GetDictionary().RemoveKey(PdfName("Version"));
  }

  m_pSignatureObj->GetDictionary().AddKey(PdfName("Version"), szVersion);
}
