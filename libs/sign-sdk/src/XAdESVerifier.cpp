...
CXAdESVerifier::CXAdESVerifier(void) : m_pXAdESDoc(NULL) {
...
void CXAdESVerifier::parseSignatureNode(xmlXPathContextPtr xpathCtx,
  ...
  *pXAdESDoc->ppSignatures = new SignatureInfo[pXAdESDoc->nSignatures];
  ...
  for (int i = 0; i < signatureNodes->nodeNr; i++) {
    ...
    // SignatureMethod
    sprintf(szPath, "//ds:Signature[@Id='%s']/ds:SignedInfo/ds:SignatureMethod",
            id);
    path = BAD_CAST szPath;
    ...
    if (xpathObj) {
      if (xpathObj->nodesetval->nodeNr > 0) {
        ...
        char* szContent = (char*)signatureMethodNode->properties[0]
                              .children[0]
    ...
    if (xpathObj) {
      if (xpathObj->nodesetval->nodeNr > 0) {
        ...
        (*pXAdESDoc->ppSignatures[i])
            .sigValue.append((BYTE*)szContent, strlen(szContent));
      }
    ...
    if (xpathObj) {
      if (xpathObj->nodesetval->nodeNr > 0) {
        ...
        char* szContent =
            (char*)xmlNodeGetContent(certificateNodeSet->nodeTab[0]);
