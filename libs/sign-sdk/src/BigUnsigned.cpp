...
void BigUnsigned::multiply(const BigUnsigned &a, const BigUnsigned &b) {
  ...
  for (i = 0; i < len; i++) blk[i] = 0;
  // For each block of the first number...
  for (i = 0; i < a.len; i++) {
    // For each 1-bit of that block...
    for (i2 = 0; i2 < N; i2++) {
      if ((a.blk[i] & (Blk(1) << i2)) == 0) continue;
      /*
...
void BigUnsigned::divideWithRemainder(const BigUnsigned &b, BigUnsigned &q) {
  ...
  /*
   * If *this.len < b.len, then *this < b, and we can be sure that b doesn't go
   * into *this at all.  The quotient is 0 and *this is already the remainder
   * (so leave it alone).
   */
  if (len < b.len) {
    q.len = 0;
...
void BigUnsigned::bitAnd(const BigUnsigned &a, const BigUnsigned &b) {
  ...
  allocate(len);
  Index i;
  for (i = 0; i < len; i++) blk[i] = a.blk[i] & b.blk[i];
  zapLeadingZeros();
}
...
void BigUnsigned::bitOr(const BigUnsigned &a, const BigUnsigned &b) {
  ...
  allocate(a2->len);
  for (i = 0; i < b2->len; i++) blk[i] = a2->blk[i] | b2->blk[i];
  for (; i < a2->len; i++) blk[i] = a2->blk[i];
  len = a2->len;
