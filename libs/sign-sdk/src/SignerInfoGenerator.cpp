...
void CSignerInfoGenerator::getSignedAttributes(UUCByteArray &signedAttribute,
        bool counterSignature,
        bool signingTime) {
    CASN1SetOf authAttributes;

    // char* szhex;
    if (!counterSignature) // nella countersignature il content type non deve
        // essere presente
    {
    ...
    // sequence.addElement(CIssuerAndSerialNumber(*m_pIssuer, *m_pSerialNumber,
    // true));
...
void CSignerInfoGenerator::buildUnsignedAttributes() {
    ...
    if (m_timeStampToken.getLength() > 0) {
        CASN1Sequence v;
        v.addElement(
            CASN1ObjectIdentifier(szTimestampTokenOID)); // id-aa-timeStampToken
        CASN1SetOf tst;
