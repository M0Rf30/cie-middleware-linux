...
void CToken::SelectMF() {
  ...
  StatusWord sw;
  if ((sw = Transmit(apdu, NULL)) != 0x9000) throw scard_error(sw);
}

ByteDynArray CToken::BinaryRead(WORD start, BYTE size) {
  init_func DWORD dwBase = 0;
  std::vector<BYTE> dt;
...
StatusWord CToken::Transmit(ByteArray &apdu, ByteDynArray *resp) {
  ...
  if (res !=
      SCARD_S_SUCCESS)  // la smart card è stata estratta durante l'operazione
...
void CToken::setTransmitCallback(TokenTransmitCallback func, void *data) {
  init_func transmitCallback = func;
  transmitCallbackData = data;
}

void CToken::setTransmitCallbackData(void *data) {
  init_func transmitCallbackData = data;
}

void *CToken::getTransmitCallbackData() { return transmitCallbackData; }
