...
UINT CASN1Object::getLength() const { return m_value.getLength(); }

const UUCByteArray* CASN1Object::getValue() const { return &m_value; }
...
int CASN1Object::parseLen(UUCBufferedReader& reader, BYTE* pbtTag,
  ...
  if (btLenRead == 0x80) {  // indefinite-length
    ...
    if (pValue) pValue->append(buffer.getContent(), buffer.getLength());
