...
static int (*orig_pkey_rsa_sign)(EVP_PKEY_CTX *ctx, unsigned char *sig,
                                 size_t *siglen, const unsigned char *tbs,
                                 size_t tbslen);
static int (*orig_pkey_rsa_decrypt_init)(EVP_PKEY_CTX *ctx);
static int (*orig_pkey_rsa_decrypt)(EVP_PKEY_CTX *ctx, unsigned char *out,
...
static const ENGINE_CMD_DEFN cie_engine_cmd_defns[] = {
    {CMD_LOAD_CERT_CTRL, "LOAD_CERT_CTRL", "Get the certificate from CIE",
...
int cie_rsa_priv_enc(int flen, const unsigned char *from, unsigned char *to,
                     RSA *rsa, int padding) {
...
static int cie_pkey_rsa_decrypt(EVP_PKEY_CTX *evp_pkey_ctx, unsigned char *out,
                                size_t *outlen, const unsigned char *in,
                                size_t inlen) {
...
static int cie_pkey_rsa_sign(EVP_PKEY_CTX *evp_pkey_ctx, unsigned char *sig,
                             size_t *siglen, const unsigned char *tbs,
                             size_t tbslen) {
  ...
  if (!evp_pkey_ctx) return 0;

  if (EVP_PKEY_CTX_get_signature_md(evp_pkey_ctx, &sig_md) <= 0) return 0;

  if (tbslen != (size_t)EVP_MD_size(sig_md)) return 0;
...
typedef int (*compute_key_fn)(unsigned char **, size_t *, const EC_POINT *,
                              const EC_KEY *);
#else
...
int cie_ecdh_compute_key(unsigned char **a, size_t *b, const EC_POINT *c,
                         const EC_KEY *d) {
...
void *cie_get_ec_key_method(void) { return NULL; }
...
static int cie_engine_ctrl(ENGINE *engine, int cmd, long i, void *p,
                           void (*f)(void)) {
...
static EVP_PKEY_METHOD *cie_pkey_method_rsa() {
  ...
  EVP_PKEY_meth_get_sign(orig_meth, &orig_pkey_rsa_sign_init,
                         &orig_pkey_rsa_sign);
  EVP_PKEY_meth_get_decrypt(orig_meth, &orig_pkey_rsa_decrypt_init,
                            &orig_pkey_rsa_decrypt);

  new_meth = EVP_PKEY_meth_new(EVP_PKEY_RSA, EVP_PKEY_FLAG_AUTOARGLEN);

  EVP_PKEY_meth_copy(new_meth, orig_meth);

  EVP_PKEY_meth_set_sign(new_meth, orig_pkey_rsa_sign_init, cie_pkey_rsa_sign);
  EVP_PKEY_meth_set_decrypt(new_meth, orig_pkey_rsa_decrypt_init,
                            cie_pkey_rsa_decrypt);

  return new_meth;
}

int cie_pkey_meths(ENGINE *e, EVP_PKEY_METHOD **pmeth, const int **nids,
                   int nid) {
