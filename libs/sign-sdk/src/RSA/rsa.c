...
static int rsapublicfunc PROTO_LIST((unsigned char *, unsigned int *,
                                     unsigned char *, unsigned int,
                                     R_RSA_PUBLIC_KEY *));
static int rsaprivatefunc PROTO_LIST((unsigned char *, unsigned int *,
                                      unsigned char *, unsigned int,
                                      R_RSA_PRIVATE_KEY *));
                     ...
                     R_RSA_PUBLIC_KEY *publicKey) /* RSA public key */
{
  ...
  if (pkcsBlockLen != modulusLen) return (RE_LEN);

  /* Require block type 1. */

  if ((pkcsBlock[0] != 0) || (pkcsBlock[1] != 1)) return (RE_DATA + 0x2000);
  ...
  if (pkcsBlock[i++] != 0) return (RE_DATA + +0x3000);

  *outputLen = modulusLen - i;

  if (*outputLen + 11 > modulusLen) return (RE_DATA + 0x4000);
                      ...
                      R_RSA_PRIVATE_KEY *privateKey) /* RSA private key */
{
  ...
  *(pkcsBlock + 1) = 1;
  ...
  for (i = 2; i < modulusLen - inputLen - 1; i++) *(pkcsBlock + i) = 0xff;
                      ...
                      R_RSA_PRIVATE_KEY *privateKey) /* RSA private key */
{
  ...
  status =
      rsaprivatefunc(pkcsBlock, &pkcsBlockLen, input, inputLen, privateKey);
  if (status) return (status);
  ...
  if ((*pkcsBlock != 0) || (*(pkcsBlock + 1) != 2)) return (RE_DATA);

  for (i = 2; i < modulusLen - 1; i++) /* separator */
    if (*(pkcsBlock + i) == 0) break;

  i++;
  if (i >= modulusLen) return (RE_DATA);

  *outputLen = modulusLen - i;

  if (*outputLen + 11 > modulusLen) return (RE_DATA);
...
*/
static int rsapublicfunc(unsigned char *output,   /* output block */
                         unsigned int *outputLen, /* length of output block */
                         unsigned char *input,    /* input block */
                         unsigned int inputLen,   /* length of input block */
                         R_RSA_PUBLIC_KEY *publicKey) /* RSA public key */
{
  ...
  if (NN_Cmp(m, n, nDigits) >= 0) return (RE_DATA + 0x1000);
...
static int rsaprivatefunc(unsigned char *output,   /* output block */
                          unsigned int *outputLen, /* length of output block */
                          unsigned char *input,    /* input block */
                          unsigned int inputLen,   /* length of input block */
                          R_RSA_PRIVATE_KEY *privateKey) /* RSA private key */
{
  ...
  if (NN_Cmp(c, n, nDigits) >= 0) return (RE_DATA);
