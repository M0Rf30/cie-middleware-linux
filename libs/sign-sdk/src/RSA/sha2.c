...
int sha2_file(const char *path, unsigned char output[32], int is224) {
  ...
  if ((f = fopen(path, "rb")) == NULL) return (POLARSSL_ERR_SHA2_FILE_IO_ERROR);

  sha2_starts(&ctx, is224);

  while ((n = fread(buf, 1, sizeof(buf), f)) > 0) sha2_update(&ctx, buf, n);
...
int sha2_self_test(int verbose) {
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (verbose != 0) printf("  SHA-%d test #%d: ", 256 - k * 32, j + 1);
    ...
    if (j == 2) {
      ...
      for (j = 0; j < 1000; j++) sha2_update(&ctx, buf, buflen);
    } else
  ...
  for (i = 0; i < 14; i++) {
    ...
    if (verbose != 0) printf("  HMAC-SHA-%d test #%d: ", 256 - k * 32, j + 1);

    if (j == 5 || j == 6) {
      memset(buf, '\xAA', buflen = 131);
      sha2_hmac_starts(&ctx, buf, buflen, k);
    } else
      sha2_hmac_starts(&ctx, sha2_hmac_test_key[j], sha2_hmac_test_keylen[j],
                       k);
