...
extern "C" {
void makeDigestInfo(int algid, unsigned char* pbtDigest, size_t btDigestLen,
                    unsigned char* pbtDigestInfo, size_t* pbtDigestInfoLen);
}

CCIESigner::CCIESigner(IAS* pIAS) : m_pIAS(pIAS), m_pCertificate(NULL) {}

CCIESigner::~CCIESigner(void) {
  if (m_pCertificate) delete m_pCertificate;
}
...
long CCIESigner::Sign(UUCByteArray& data, UUCByteArray& id, int algo,
  ...
  try {
    ...
    switch (algo) {
      case CKM_SHA256_RSA_PKCS:
        makeDigestInfo(NID_sha256, (unsigned char*)data.getContent(),
                       (size_t)data.getLength(), (unsigned char*)digestinfo,
                       &digestinfolen);

        break;

      case CKM_SHA1_RSA_PKCS:
        makeDigestInfo(NID_sha1, (unsigned char*)data.getContent(),
                       (size_t)data.getLength(), (unsigned char*)digestinfo,
                       &digestinfolen);
        break;
