...
void CTSAClient::SetTSAUrl(const char* szUrl) { strcpy(m_szTSAUrl, szUrl); }
...
long CTSAClient::GetTimeStampToken(UUCByteArray& digest, const char* szPolicyID,
  ...
  try {
    CTimeStampResponse tsResponse((BYTE*)tsdata.getContent(),
                                  (int)tsdata.getLength());
  ...
  if (*ppTimeStampToken == NULL) return -1;
