...
long verify_signed_document(DISIGON_VERIFY_CONTEXT* pContext,
...
long verify_pdf(DISIGON_VERIFY_CONTEXT* pContext, UUCByteArray& data,
...
long disigon_set(int option, void* value) {
  switch (option) {
    case DISIGON_OPT_CACERT_DIR:
      LOG_DBG(
          (0, "disigon_set", "set DISIGON_OPT_CACERT_DIR: %s", (char*)value));
...
void disigon_cleanup() { CCertStore::CleanUp(); }
...
long disigon_sign_set(DISIGON_CTX ctx, int option, void* value) {
  ...
  switch (option) {
    case DISIGON_OPT_IAS_INSTANCE:
      LOG_MSG((0, "disigon_sign_set", "Context: %p, Option: %s, Value: %s", ctx,
               "DISIGON_OPT_IAS_INSTANCE", (char*)value));
    ...
    case DISIGON_OPT_ALIAS:
      LOG_MSG((0, "disigon_sign_set", "Context: %p, Option: %s, Value: %s", ctx,
               "DISIGON_OPT_ALIAS", (char*)value));
      strcpy(pContext->szAlias, (char*)value);
      // pContext->pSignatureGenerator->SetAlias((char*)value);
      break;

    case DISIGON_OPT_CADES:
      LOG_MSG((0, "disigon_sign_set", "Context: %p, Option: %s, Value: %d", ctx,
               "DISIGON_OPT_CADES", (long)value));
    ...
    case DISIGON_OPT_TSA_USERNAME:
      LOG_MSG((0, "disigon_sign_set", "Context: %p, Option: %s, Value: %s", ctx,
               "DISIGON_OPT_TSA_USERNAME", (char*)value));
...
long disigon_sign_sign(DISIGON_CTX ctx) {
  ...
  LOG_MSG((0, "--> disigon_sign_sign", "pContext: %p, pdf_left: %f", pContext,
           pContext->fPdfLeft));
  ...
  if (!f) {
    LOG_ERR((0, "<-- disigon_sign_sign", "Context: %p, Error: %x, file: %s",
             pContext, DISIGON_ERROR_FILE_NOT_FOUND, pContext->szInputFile));
  ...
  if (nRes) {
    LOG_ERR(
        (0, "<-- disigon_sign_sign", "Context: %p, Error: %x", pContext, nRes));
    return nRes;
  }
  ...
  LOG_DBG((0, "disigon_sign_sign", "Context: %p, Outputfile: %s", pContext,
           pContext->szOutputFile));

  f = fopen(pContext->szOutputFile, "w+b");
  if (!f) {
    LOG_ERR((0, "<-- disigon_sign_sign", "Context: %p, Error: %x, file: %s",
             pContext, DISIGON_ERROR_FILE_NOT_FOUND, pContext->szOutputFile));
    return DISIGON_ERROR_FILE_NOT_FOUND;
  }
...
long disigon_verify_set(DISIGON_CTX ctx, int option, void* value) {
  ...
  switch (option) {
    ...
    case DISIGON_OPT_PROXY_USRPASS:
      ...
      LOG_DBG((0, "disigon_verify_set", "ProxyUsrPass: %s",
...
long disigon_verify_verify(DISIGON_CTX ctx, VERIFY_RESULT* pVerifyResult) {
  ...
  if (pContext->szInputFile[0] == 0) {
    LOG_ERR((0, "disigon_verify_verify",
             "Context: %p, Error: DISIGON_ERROR_INVALID_FILE"));
    return DISIGON_ERROR_INVALID_FILE;
  }
  ...
  LOG_DBG((0, "disigon_verify_verify", "Context: %p, FileType: %d", ctx,
           nFileType));
  ...
  switch (nFileType) {
    ...
    default:
      LOG_ERR((0, "<-- disigon_verify_verify", "Context: %p, Error: %x",
               pContext, DISIGON_ERROR_INVALID_FILE));
...
long disigon_verify_cleanup_result(VERIFY_RESULT* pVerifyResult) {
  LOG_MSG((0, "--> disigon_verify_cleanup_result", "VerifyResult: %p",
           pVerifyResult));
    ...
    case DISIGON_FILETYPE_XML:
      ...
      for (int i = 0; i < pVerifyResult->verifyInfo.pSignerInfos->nCount; i++) {
        if (pVerifyResult->verifyInfo.pSignerInfos->pSignerInfo[i].pTimeStamp) {
          if (((TS_INFO*)pVerifyResult->verifyInfo.pSignerInfos->pSignerInfo[i]
                   .pTimeStamp)
...
long verify_p7m(DISIGON_VERIFY_CONTEXT* pContext, VERIFY_INFO* pVerifyInfo) {
  ...
  if (!f) {
    LOG_ERR((0, "<-- verify_p7m",
             "Context: %p, Error: DISIGON_ERROR_FILE_NOT_FOUND, file: %s",
  ...
  try {
    ...
    if (sd.isDetached()) {
      if (pContext->szInputPlainTextFile[0] != '\0') {
        ...
        LOG_ERR((0, "<-- verify_p7m",
                 "Context: %p, Error: DISIGON_ERROR_DETACHED_PKCS7, file: %s",
                 pContext, pContext->szInputFile));
...
long disigon_get_file_from_p7m(DISIGON_CTX ctx) {
  ...
  if (nFileType != DISIGON_FILETYPE_P7M) return DISIGON_ERROR_INVALID_FILE;
...
long verify_xml(DISIGON_VERIFY_CONTEXT* pContext, VERIFY_INFO* pVerifyInfo) {
  ...
  for (int i = 0; i < signatureCount; i++) {
    ...
    strcpy(pSI->szSN,
           ((UUCByteArray*)pCert->getSerialNumber().getValue())->toHexString());
    ...
    for (int j = 0; j < count; j++) {
      ...
      const char* szoid = g_mapOIDProps.getProperty(
          (char*)oidName.getContent(), (char*)oidName.getContent());
...
SIGNER_INFO* verify_countersignature(DISIGON_VERIFY_CONTEXT* pContext,
  ...
  if (counterSignatureCount > 0) {
    ...
    for (int i = 0; i < counterSignatureCount; i++) {
      ...
      pSI->bitmask = si.verifyCountersignature(i, certificates, NULL,
                                               pSI->pRevocationInfo);
      ...
      strcpy(pSI->szSN,
             ((UUCByteArray*)cert.getSerialNumber().getValue())->toHexString());
      ...
      if (si.hasTimeStampToken()) {
        ...
        strcpy(
            pTSInfo->signerInfo.szSN,
            ((UUCByteArray*)cert.getSerialNumber().getValue())->toHexString());
        strcpy(pTSInfo->szTimeStampSerial,
               ((UUCByteArray*)tstInfo.getSerialNumber().getValue())
                   ->toHexString());
        ...
        memcpy(pTSInfo->signerInfo.pCertificate, certificate->getContent(),
        ...
        base64_encoded_size(val->getLength());
        char* szEncoded =
            base64_encode((char*)val->getContent(), val->getLength());
...
long verify_signed_document(int index, DISIGON_VERIFY_CONTEXT* pContext,
                            CSignedDocument& sd, VERIFY_INFO* pVerifyInfo) {
  ...
  for (int i = 0; i < sigCount; i++) {
    ...
    if (si.hasTimeStampToken()) {
      ...
      strcpy(pTSInfo->signerInfo.szSN,
             ((UUCByteArray*)cert.getSerialNumber().getValue())->toHexString());
      strcpy(
          pTSInfo->szTimeStampSerial,
          ((UUCByteArray*)tstInfo.getSerialNumber().getValue())->toHexString());
...
long sign_pdf(DISIGON_SIGN_CONTEXT* pContext, UUCByteArray& data) {
  ...
  if (pContext->szPdfImagePath[0] != 0 || pContext->szPdfDescription[0] != 0 ||
    ...
    if (!pContext->szPdfReason[0]) {
      ...
      if (r == 0) {
        ...
        sprintf(pContext->szPdfReason, "%s %s", giveName.c_str(),
                surname.c_str());
    ...
    sigGen.InitSignature(
        ...
        pContext->szPdfImagePath, pContext->szPdfDescription, NULL, NULL);
  } else {
  ...
  UUCByteArray signature;
  long nRes = pContext->pSignatureGenerator->Generate(signature, true,
                                                      pContext->bVerifyCert);
  if (nRes) {
...
long verify_pdf(DISIGON_VERIFY_CONTEXT* pContext, UUCByteArray& data,
  ...
  for (int i = 0; i < signatureCount; i++) {
    ...
    char sigType[256];
    pSI->bitmask =
        pdfVerifier.VerifySignature(i, NULL, sigType, pSI->pRevocationInfo);
...
long verifyTST(CTimeStampToken& tst, TS_INFO* pTSInfo, BOOL bVerifyCRL) {
  ...
  strcpy(pTSInfo->signerInfo.szSN,
         ((UUCByteArray*)tsacert.getSerialNumber().getValue())->toHexString());
  strcpy(pTSInfo->szTimeStampSerial,
         ((UUCByteArray*)tstInfo.getSerialNumber().getValue())->toHexString());
