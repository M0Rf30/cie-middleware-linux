...
void CounterSignatureGenerator::setSigningCertificate(const BYTE* certificate,
                                                      int certlen,
                                                      const BYTE* certHash,
                                                      int certHashLen) {
  m_signingCertificate.append(certificate, certlen);
  m_signerInfoGenerator.setSigningCertificate(certificate, certlen, certHash,
                                              certHashLen);
}
...
void CounterSignatureGenerator::setSignature(const BYTE* signature,
                                             int siglen) {
  m_signerInfoGenerator.setSignature(signature, siglen);
}

void CounterSignatureGenerator::setTimestampToken(const BYTE* timestampToken,
                                                  int tstlen) {
  m_signerInfoGenerator.setTimestampToken(timestampToken, tstlen);
}

void CounterSignatureGenerator::getSignedAttributes(
    UUCByteArray& signedAttribute) {
...
void CounterSignatureGenerator::toByteArray(UUCByteArray& pkcs7SignedData) {
  ...
  CASN1Sequence v;
  v.addElement(
      CASN1ObjectIdentifier(szCounterSignatureOID));  // id-aa-timeStampToken
  ...
  if (i == m_certificates.size()) m_certificates.addElement(cert);
