...
class UUCHashtable {
 private:
  ...
  void rehash() {
    ...
    int nIndex;
    for (nIndex = m_nSize - 1; nIndex > 0 && oldTable[nIndex] == NULL;
         nIndex--) {
    }
 ...
 public:
  ...
  POS getFirstPosition() {
    for (m_nIndex = m_nSize - 1; m_nIndex > 0 && m_table[m_nIndex] == NULL;
         m_nIndex--) {
    }
    ...
    if (pos) {
      ...
      if (pos == NULL) {
        if (m_nIndex == 0) {
          ...
          for (m_nIndex--; m_nIndex > 0 && m_table[m_nIndex] == NULL;
               m_nIndex--) {
          }
